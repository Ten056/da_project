...
function getData() {
  // apiのURL
  const bookUrl = `http://localhost:3333/books/0`
  const userUrl = `http://localhost:3333/users/0`
...
const fontRate = isMobileView() ? 0.6 : 1;  // スマートフォンでは80%に、それ以外では100%に設定
const barWidth = isMobileView() ? window.innerWidth * 0.8 : 400;  // スマートフォンでは80%に、それ以外では100%に設定
